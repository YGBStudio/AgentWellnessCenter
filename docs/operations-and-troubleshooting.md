...
| Runtime error says D1 binding `DB` is required | App is not running through the OpenNext Cloudflare path or binding is missing. | Check `wrangler.toml`, `next.config.js`, and the run command. |
| Booking page says demo care data is missing | Seed data has not initialized or reset did not run. | Visit a server page, check database initialization, or run the app through the expected D1 binding. |
...
| Static assets or styles fail in Workers preview | OpenNext assets output or `ASSETS` binding is missing. | Re-run `npm run workers:build` and check `wrangler.toml`. |
